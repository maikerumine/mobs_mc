...
--REVISED 20160516 maikerumine for esmobs
...
local enable_mob_allbones = false
	...
	elseif self.state == "attack" then
		...
		or (self.attack_type == "dogshoot" and dist > self.reach and dogswitch(self) == 0) then
			...
			and math.random(1, 100) <= 60 then
				...
				local v = ent.velocity or 1 -- or set to default  --seems to be buggy mc
				ent.switch = 1
		...
		if self.name == "mobs:pig" and self.saddle == "yes" and self.driver then
			...
			if item:get_name() == "mobs:carrotstick" then
				...
				if stack:get_wear() > 65400 then
					stack = {name = "fishing:pole_wood", count = 1}
				end
...
minetest.register_entity(name, {
	...
	mesh = def.mesh,
	textures = def.textures,  -- maikerumine texture code
